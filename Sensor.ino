...
int sensorData() {
  ...
  int distanceModifier = 10;             // sets a value to compare the modulus in data sorting later, prevents errant inputs from being considered, does not add to an error count like 0s
  ...
  for (int i = 0; i < loops; i++) {
    ...
    if (distance != 0) {
      if (avgDistance == 0 && i < 3) {  // this condition only runs if no avgDistance has not been set yet (besides initial 0) allows the establishment of a baseline on startup for error correction
        rollingInput[i] = distance;
        Serial.print("Resetting error correction input, avgDistance was set to 0");Serial.print("\t");Serial.print("rollingInput at index [");Serial.print(i);Serial.print("] = ");Serial.println(rollingInput[i]);
        ...
        if (distance < avgDistance && avgDistance - distance > distanceModifier) {
          ...
          rollingInput[i] = distance;
          Serial.print("rollingInput at index [");Serial.print(i);Serial.print("] = ");Serial.println(rollingInput[i]);
        }
      ...
      zeroCount++;
      Serial.println(" 0! ");Serial.print("Zero count = ");Serial.println(zeroCount);
...
int averageOfLoops(int loops, int rollingInput[]) {
  int loopAvg = 0;  // used as return value for function, calculate avg of rollingInput at end of function, otherwise allow to return 0
  for (int i = 0; i < loops; i++) {
