...
void refreshDisplay()
{
	...
	{ // in my setup you may notice that we can never reach a ditance less than 10, by design this is more flexible than needed so unexpected inputs don't cause errors
		lcd.setCursor(4, 0);
		lcd.print("  ");
		lcd.print(avgDistance);
	}
	else if (avgDistance < 100)
	{ // in my setup we should only ever recieve 2 digit or greater measurements without triggering any mechanisms
		lcd.setCursor(4, 0);
		lcd.print(" ");
		lcd.print(avgDistance);
	}
